...
sectors.file<-"D:/Work/Stocks/sectors.csv"
...
# ticker_new<-c("GODREJCP.NS", "MARICO.NS", "COLPAL.NS")
...
for(i in 1:length(ticker_new)){
  prices <- adjust.prices(substr(ticker_new[i], 1, nchar(ticker_new[i])-3), Sys.Date() - 180, Sys.Date(),corp.actions )
  if(i==1) {
...
for(i in 1:nrow(main.output)){
  if(sectors$Industry[sectors$Symbol==as.character(main.output$series1[i])] == sectors$Industry[sectors$Symbol==as.character(main.output$series2[i])]){
    tryCatch({
