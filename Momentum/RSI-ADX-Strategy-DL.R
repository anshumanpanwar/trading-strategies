...
validation$predictions<-as.data.frame(h2o.predict(model, validation_h2o))[,1]
testing$predictions<-as.data.frame(h2o.predict(model, testing_h2o))[,1]
dl.pred<-as.data.frame(h2o.predict(model, testing_h2o))[,-1]

#Multi class log loss function. Ranges from 0-inf
# multi.log.loss<-MultiLogLoss(dl.pred, as.character(testing$label))
#Accuracy
...
for ( i in as.character(levels(testing[, 'label']) )) {
  ...
  print(i)
  roc_obj <- roc(ifelse(testing$label==i,1,0), dl.pred[,i])
  auc(roc_obj)
...
pred.val<-as.data.frame(h2o.predict(model, validation_h2o))[,-1]  #probability matrix on validation set
...
print(drop.classes)
dl.pred1<-dl.pred
dl.pred1<- dl.pred1[ , !(colnames(dl.pred1) %in% drop.classes)]
#Comparing if the columns in the dl.pred matrix are aligned with the labels code in rows for roc.df before performing a matrix-vector multiplication
roc.df<-roc.df[roc.df$cutoff<=1, ]
if(sum((colnames(dl.pred1)==roc.df[roc.df$cutoff!=0,"label"])==FALSE) >0 ) { #checking for mis-alignment
...
dl.pred1<-sweep(dl.pred1,MARGIN=2,roc.vector,`*`)   #Performing vector-matrix multipplication. Vector here is inverse of cutoffs
...
testing$predicted<- apply(dl.pred1, 1, function(row){ if(sum(row>=1)>=1) colnames(dl.pred1)[which.max(row)] else "Unknown"})  
