...
run.strategy<-function(ticker, prices, nifty, start.date, end.date){
  ...
  macd1$bars.firstderivative<-diff(macd1$bars)
  macd1$bars.firstderivative.sma<-SMA(macd1$bars.firstderivative, n=2)
  ...
  while(i<=nrow(macd)-2){   #MACD crossing takes place at close of -1 and trade closes at open of 0
      ...
      if(drawdown.exit){  #exit on drawdown
        if(drawdown>drawdown.threshold){
          if(direction==1){
            ...
            temp.row<-list(ticker, as.character(index(macd)[start.index]), as.character(index(macd)[end.index]), direction, (end.index-start.index), ret, cons.ret, drawdown, ifelse((ret+cons.ret)>0,1,0))
    ...
    if(i==(nrow(macd)-2) & automata.state==1){  #capture ongoing trend
      ...
      if(direction==0){
        temp.row2<-list(ticker, stock.beta, stock.rsquare, stock.ret.var, as.character(index(macd)[start.index]), as.character(index(macd)[end.index]), direction, dirty.exit, (end.index-start.index), ret, cons.ret, last.ret, last.seq, drawdown, 
                        macd$macd[start.index], -macd$bars.firstderivative.sma[start.index], adx$ADX[start.index]*ifelse(adx$DIn[start.index] > adx$DIp[start.index], 1, -1), adx$firstderivative.sma[start.index]*ifelse(adx$DIn[start.index] > adx$DIp[start.index], 1, -1), adx2$ADX[start.index]*ifelse(adx2$DIn[start.index] > adx2$DIp[start.index], 1, -1),
                        smi$SMI[start.index], 100-rsi$EMA[start.index], bollinger.width, bollinger.percentage, market.vol, nifty.macd$macd[start.index], -nifty.macd$bars[start.index], nifty.smi$SMI[start.index],
                        nifty.adx$ADX[start.index]*ifelse(nifty.adx$DIn[start.index] > nifty.adx$DIp[start.index], 1, -1), stock.beta*nifty.adx$ADX[start.index]*ifelse(nifty.adx$DIn[start.index] > nifty.adx$DIp[start.index], 1, -1), 
                        stock.rsquare*nifty.adx$ADX[start.index]*ifelse(nifty.adx$DIn[start.index] > nifty.adx$DIp[start.index], 1, -1), nifty.adx2$ADX[start.index]*ifelse(nifty.adx2$DIn[start.index] > nifty.adx2$DIp[start.index], 1, -1), stock.beta*nifty.adx2$ADX[start.index]*ifelse(nifty.adx2$DIn[start.index] > nifty.adx2$DIp[start.index], 1, -1), 
  ...
  header<-c("stock.ticker", "stock.beta", "stock.rquare", "stock.ret.var" ,"start.date", "end.date", "direction", "dirty.exit", "seq length", "returns", "Cons-Return", "last.ret", "last.seq", "drawdown", "macd", "macd.dx.sma", "adx", "adx.dx.sma", "adx.longer", "smi", "rsi", "bollinger.width", "bollinger.percentage", "Nifty Vol", "Nifty MACD", "NIFTY MACD BAR", "NIFTY SMI" ,"Nifty ADX", "beta X Nifty ADX", "rSquare X Nifty ADX", "Nifty ADX2", "beta X Nifty ADX2", "rSquare X Nifty ADX2", "viable")
...
for(i in 1:length(tickers_100)){
  ...
  result<-run.strategy(tickers_100[i], prices, nifty, Sys.Date() - lookback, Sys.Date()-1)
...
write.table(output, file = paste("D://Work/Stocks/MACD/macd-", "results-simple-macd-exit-top",".csv", sep=""), append = FALSE, col.names = TRUE, sep = ",", row.names=FALSE)
write.table(output2, file = paste("D://Work/Stocks/MACD/macd-", "ongoing-simple-macd-exit-top",".csv", sep=""), append = FALSE, col.names = TRUE, sep = ",", row.names=FALSE)

cutoff.date<-"2017-05-31"
training<-output[output$end.date<cutoff.date,]
testing<-output[output$start.date>cutoff.date,]
write.table(training[training$last.seq>0,], "D://Work/Stocks/MACD/macd-train10.csv",append = FALSE, col.names = TRUE, sep = ",", row.names=FALSE)
write.table(testing, "D://Work/Stocks/MACD/macd-test10.csv",append = FALSE, col.names = TRUE, sep = ",", row.names=FALSE)
