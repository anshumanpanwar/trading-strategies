...
entry.percentage<-3
exit.percentage<-30   #exit at 50%+- exit percentage
smi.lookback<-200
...
run.strategy<-function(ticker, prices, nifty, nifty.adx, nifty.smi, start.date, end.date){
  ...
  while(i<=(nrow(smi1)-1)){
    if(first.flag | i%%10 ==0){
      tmp<-smi1[max(i-smi.lookback,1):(i-1),1]  #last 200 tradings days SMI use to find levels
      tmp<-sort(as.vector(tmp))
      uptrend.entry<-min(-40,tmp[floor(length(tmp)*entry.percentage*0.01)])
      downtrend.entry<-max(40,tmp[ceiling(length(tmp)*(100-entry.percentage)*0.01)])
      uptrend.exit<-max(10,tmp[floor(length(tmp)*(50+exit.percentage)*0.01)])
      downtrend.exit<-min(-10,tmp[ceiling(length(tmp)*(50-exit.percentage)*0.01)])
      printf("date %s, i= %d, uptrend.entry=%f, downtrend.entry=%f, uptrend.exit=%f, downtrend.exit=%f",as.Date(index(prices2)[i]), i, uptrend.entry, downtrend.entry, uptrend.exit, downtrend.exit)
      # printf("low index %f", floor(length(tmp)*entry.percentage*0.01))
      # printf("High index %f",ceiling(length(tmp)*(100-entry.percentage)*0.01))
      first.flag<-FALSE
    }
    if(automata.state==1){
      if(direction==1){
    ...
    if(automata.state==0 & as.numeric(smi1[i,1])<uptrend.entry) {  #smi below uptrend entry levels
      printf("starting trend %s , direction %d",as.Date(index(prices2)[i]), 1)
      direction<-1
      automata.state<-1
      start.index<-i
      drawdown<-0
      entry.level<-as.numeric(smi1[i,1])
    }
    else if(automata.state==0 & as.numeric(smi1[i,1])>downtrend.entry) {  #smi above downtrend entry levels
    ...
    else if(automata.state==1 & direction==1 & as.numeric(smi1[i,1])>uptrend.exit) {  #smi above exit levels
      automata.state<-0
      end.index<-i
      exit.level<-as.numeric(smi1[i,1])
  ...
  header<-c("stock.ticker", "ret.var", "stock.beta", "stock.rsquare", "start.date", "end.date", "direction", "seq length", "returns", "Cons-Return", "drawdown", "entry.level", "exit.level", "entry.percentage", "exit.percentage", "NIFTY.ADX", "NIFTY.SMI", "adf.result1", "pp.result1", "adf.result2", "pp.result2", "viable")
...
entry<-c(3,5)
exit<-c(0,10,20,30,40,45)
periods<-c(100,200,300,400,500)
  ...
  for (n in exit){
    ...
    for(p in periods){
      ...
      for(i in 1:length(tickers_100)){
        ...
        prices <- adjust.prices(substr(tickers_100[i], 1, nchar(tickers_100[i])-3), Sys.Date() - lookback- additional.lookback, Sys.Date()-4,corp.actions )
        result<-run.strategy(tickers_100[i], prices, nifty, nifty.adx, nifty.smi, Sys.Date() - lookback, Sys.Date()-4)
        if(!is.null(result)){
      ...
      write.table(summary, "D://Work/Stocks/smi-summary6.csv",append = FALSE, col.names = TRUE, sep = ",", row.names=FALSE)
    }
  }
}
colnames(summary)<-c("Entry", "Exit", "Lookback", "avg return", "avg cons.ret", "Avg seq.len", "Count", "1 avg return", "1 Avg seq.len", "1 count", "0 avg return", "0 Avg seq.len", "0 count")
